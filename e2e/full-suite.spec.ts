...
test.describe('02-Authorization', () => {
  ...
  test('user burns own story → 200 + content replaced with memorial', async ({ request }) => {
    ...
    expect(check.status).toBe(200); // 故事保留为纪念墓碑
...
test.describe('09-UI', () => {
  ...
  test('burn button visible to author, hidden for non-author + modal interactions', async ({ page }) => {
    ...
    // Burn lives inside the "more actions" menu — open it and confirm the item appears
    const actionToggle = page.locator('.action-toggle');
    await actionToggle.click();
    await page.waitForTimeout(300);
    const burnItem = page.locator('.action-menu-item--danger');
    await expect(burnItem).toBeVisible({ timeout: 3000 });

    // ── Burn modal: open, close via overlay, escape, keep button ──
    await burnItem.click();
    ...
    // ── Now login as user B - burn item should NOT be visible for A's story ──
    await uiLoginFast(page, S.userB.token, S.userB.id, EMAIL_B);
    await page.goto(`/story/${S.storyByA.id}`);
    await page.waitForTimeout(2000);
    await page.locator('.action-toggle').click();
    await page.waitForTimeout(300);
    const notVisible = await page.locator('.action-menu-item--danger').isVisible().catch(() => false);
  ...
  test('create page: inputs visible, validation, submit button disabled when empty', async ({ page }) => {
    ...
    // Validation message is now i18n'd (zh/en/…) — assert presence, not the exact text
