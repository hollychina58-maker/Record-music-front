...
export const useAuthStore = create<AuthState>()(
  persist(
    (set, get) => ({
      ...
      login: async (email: string, password: string) => {
        ...
        try {
          const res = await fetch(`${API_BASE}/auth/login`, {
            ...
            credentials: 'include', // 3-4: httpOnly cookie session
      ...
      logout: async () => {
        try {
          // 3-4: clear the httpOnly session cookie server-side
      ...
      fetchCurrentUser: async () => {
        // 3-4: session is an httpOnly cookie — always attempt, even with no in-memory token
        const { token } = get();
    ...
    }),
    {
      name: 'auth-storage',
      partialize: (state) => ({
        // 3-4: JWT no longer persisted — the httpOnly cookie is the session
        user: state.user,
