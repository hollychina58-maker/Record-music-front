...
export function MySpacePage() {
  ...
  const loadData = async () => {
    ...
    } catch (err) { console.error('[MySpace] Action failed:', err);
      setLoadError(true);
    } finally {
      setLoading(false);
    }
        ...
        <div className="profile-main">
          <div className="profile-info">
            <h2 className="profile-name">
              {profile?.nickname}
              {subLabel && profile?.subscription && <span className={`profile-badge badge--${profile.subscription.planType}`}>{subLabel}</span>}
            </h2>
          ...
          <div className="profile-subscription">
            ...
            <span className="sub-expiry">
              {profile.subscription.musicRemaining !== null
                ...
                : t('payment.unlimited')}
              &nbsp;&middot;&nbsp;
              {t('profile.validUntil')} {formatDate(profile.subscription.expiresAt)}
            </span>
          </div>
        )}
      </section>
              ...
              {stories.map((s, i) => (
                <div key={s.id} className="timeline-item" style={{ animationDelay: `${0.2 + i * 0.1}s` }}>
                  ...
                  <time className="timeline-date">
                    {formatDate(s.created_at, { month: 'long', day: 'numeric' })}
                  </time>
