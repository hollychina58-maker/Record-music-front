...
export function MessageDetailPage() {
  ...
  const loadMessages = async () => {
    try {
      const d = await apiService.clientGet<{ data: Message[]; isBlocked: boolean }>('/messages/' + otherId + '?limit=30');
      setMessages(d.data ?? []);
      ...
      if (d.data?.length > 0) {
        ...
        try {
          const p = await apiService.clientGet<{ data: { nickname: string } }>('/users/' + otherId + '/profile');
          setOtherNickname(p.data?.nickname || '');
        } catch (err) { console.error('[MessageDetail] Profile fetch failed:', err); }
      }
    } catch { /* */ }
