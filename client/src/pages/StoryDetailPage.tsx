...
export function StoryDetailPage() {
  ...
  const loadStory = async (storyId: number) => {
    try {
        ...
        .then((tracks) => {
          if (tracks.length > 0) {
            ...
            if (track.status === 'completed') {
              ...
              // 需求2：pending 一律显示"生成中"，不因超时判失败——
              // 只有后端明确返回 failed/expired 才显示"重新生成"。
              setMusic(track);
      ...
      console.error('[StoryDetail] Load failed:', err);
      setLoadError(true);
    } finally {
      setLoading(false);
    }
  ...
  const handleBurn = async () => {
    ...
    try {
      ...
      // H8: burning is destructive — the user must know it did NOT happen
      console.error('[Burn] Failed:', err);
      addToast('error', '焚烧失败，请稍后重试', { duration: 4000 });
    }
  };
  ...
  if (loadError && !story) {
    // 3-14: network failure is distinct from 404
    return (
      <div className="detail-page">
