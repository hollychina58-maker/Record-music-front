...
export function CreateStoryPage() {
  ...
  const validateField = (field: 'title' | 'content', value: string) => {
    if (field === 'title' && value.trim().length > 0 && value.trim().length < 2) {
      return t('create.error.titleTooShort');
    }
    if (field === 'content' && value.trim().length > 0 && value.trim().length < 10) {
      return t('create.error.contentTooShort');
    }
    return '';
  };
    ...
    try {
      ...
      if (withMusic && user) {
        try {
          ...
          // 2-7: guard against a corrupted localStorage value (would otherwise throw and
          // block navigation after the story was already created)
          let pending: Array<{ musicId: number; storyId: number; createdAt: number }> = [];
          try { pending = JSON.parse(localStorage.getItem('mo_pending_music') || '[]'); } catch { pending = []; }
          ...
          // 需求1：生成后立即提示，稍后异步通知结果（不再让用户盯着轮询等待）
          addToast('info', t('create.musicGenerating'), { duration: 5000 });
